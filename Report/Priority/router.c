...
int socketFD, socketFD2;
char *IP1 = "169.254.204.75", *IP2 = "169.254.204.75", *IP3 = "169.254.204.75", *IP;
...
void *threadfunction1()
	{
		...
		while(1) {	
			...
			if (msg[128] == '1') {
				...
				memcpy(current->msg, msg, 141*sizeof(char));		
				current->prev = head1;
...
void *threadfunction2() {
	...
	while (1) {
		if (tail1->prev->prev != head1) {
			...
			middle1->next = tail1;
			tail1->prev = middle1;
			memcpy(tosend, middle1->msg, 141*sizeof(char));	
			pktsent1 += 1;		
		} 
			...
			middle2->next = tail2;
			tail2->prev = middle2;
			memcpy(tosend, middle2->msg, 141*sizeof(char));			
			pktsent2 += 1;
		}
		else {
			continue;
