...
void *threadfunction1(){
	...
	if(getaddrinfo("169.254.204.75", "56800", &hints, &servinfo) != 0 || servinfo == NULL){
...
void *threadfunction2() {
	...
	while (ack <= NUM_PACKETS) { 
		...
		memcpy(returned, &ack, 4);
		memcpy(returned+4, &timestamp,8);

		if (sendto(socketFD, &returned, 12, 0, servinfo->ai_addr, servinfo->ai_addrlen) < 0) {
